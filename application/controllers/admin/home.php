...
class Home extends Admin_Controller {
	...
	public function index() {
		...
		$recent_users = $user->order_by('last_login', 'desc')->limit(10)->get();
		$latest_orders = $this->db->select('orders.*, (SELECT username FROM users WHERE users.id = orders.user_id) AS username', FALSE)->from('orders')->order_by('id', 'desc')->limit(10)->get()->result_object();
		$article = new Article;
		$latest_articles = $article->where('status', 'published')->limit(5)->get();
		$post = new Post;
		$pending_posts = $post->where('is_approved', '0')->limit(5)->get();
		...
		if(setting('google_analytics.username') AND setting('google_analytics.password') AND 
		   setting('google_analytics.site_id'))
		{
			$analytics = TRUE;
		}
